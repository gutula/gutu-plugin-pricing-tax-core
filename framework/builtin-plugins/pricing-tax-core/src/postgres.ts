export type PricingTaxCoreSqlOptions = {
  schemaName?: string;
  dropSchema?: boolean;
};

export function buildPricingTaxCoreMigrationSql(options: PricingTaxCoreSqlOptions = {}): string[] {
  const schemaName = normalizeIdentifier(options.schemaName ?? "pricing_tax_core", "schemaName");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getPricingTaxCoreLookupIndexName()} ON ${schemaName}.primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getPricingTaxCoreStatusIndexName()} ON ${schemaName}.exception_records (tenant_id, status, severity);`
  ];
}

export function buildPricingTaxCoreRollbackSql(options: PricingTaxCoreSqlOptions = {}): string[] {
  const schemaName = normalizeIdentifier(options.schemaName ?? "pricing_tax_core", "schemaName");
  const dropSchema = options.dropSchema ?? schemaName !== "pricing_tax_core";
  return [
...
export function getPricingTaxCoreLookupIndexName(): string {
  return "pricing_tax_core_primary_lookup_idx";
}

export function getPricingTaxCoreStatusIndexName(): string {
  return "pricing_tax_core_exception_status_idx";
}
