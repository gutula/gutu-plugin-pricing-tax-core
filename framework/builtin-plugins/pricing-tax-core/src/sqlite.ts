export type PricingTaxCoreSqliteOptions = {
  tablePrefix?: string;
};

export function buildPricingTaxCoreSqliteMigrationSql(options: PricingTaxCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "pricing_tax_core_");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getPricingTaxCoreSqliteLookupIndexName(tablePrefix)} ON ${tablePrefix}primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getPricingTaxCoreSqliteStatusIndexName(tablePrefix)} ON ${tablePrefix}exception_records (tenant_id, status, severity);`
  ];
}

export function buildPricingTaxCoreSqliteRollbackSql(options: PricingTaxCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "pricing_tax_core_");
  return [
...
export function getPricingTaxCoreSqliteLookupIndexName(tablePrefix = "pricing_tax_core_"): string {
  return `${normalizePrefix(tablePrefix)}primary_lookup_idx`;
}

export function getPricingTaxCoreSqliteStatusIndexName(tablePrefix = "pricing_tax_core_"): string {
