...
describe("pricing-tax-core lifecycle integration", () => {
  const previousStateDir = process.env.GUTU_STATE_DIR;
  let stateDir = "";

  beforeEach(() => {
    stateDir = mkdtempSync(join(tmpdir(), "pricing-tax-core-"));
  ...
  it("creates, advances, and reconciles a governed business record", async () => {
    const created = await createPrimaryRecord({
      ...
      actorId: "actor_admin",
      recordId: "pricing-tax-core:demo",
      title: "Pricing & Tax Core Demo",
      ...
      correlationId: "pricing-tax-core:corr",
      processId: "pricing-policy-lifecycle:demo"
    });
    ...
    const reconciled = await reconcilePrimaryRecord({
      ...
      actorId: "actor_admin",
      recordId: "pricing-tax-core:demo",
      exceptionId: "pricing-tax-core:exception",
    ...
    const amended = await amendPrimaryRecord({
      ...
      actorId: "actor_admin",
      recordId: "pricing-tax-core:demo",
      amendedRecordId: "pricing-tax-core:demo:amended",
      expectedRevisionNo: 5,
      title: "Pricing & Tax Core Demo Amendment",
      reasonCode: "commercial-correction"
    });
    expect(amended.amendedRecordId).toBe("pricing-tax-core:demo:amended");

    const reversed = await reversePrimaryRecord({
      tenantId: "tenant_demo",
      actorId: "actor_admin",
      recordId: "pricing-tax-core:demo:amended",
      reversalRecordId: "pricing-tax-core:demo:amended:reversal",
      expectedRevisionNo: 1,
      reasonCode: "cancel-amendment"
    });
    expect(reversed.reversalRecordId).toBe("pricing-tax-core:demo:amended:reversal");
