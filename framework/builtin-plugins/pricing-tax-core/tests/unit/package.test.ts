...
import { businessFlowDefinitions, publishPriceList, publishTaxRules, evaluateCommercialPolicy, placeRecordOnHold, releaseRecordHold, amendRecord, reverseRecord } from "../../src/flows/catalog";
...
describe("plugin manifest", () => {
  it("keeps a stable package id and business contract surface", () => {
    expect(manifest.id).toBe("pricing-tax-core");
    expect(manifest.kind).toBe("plugin");
    expect(manifest.publicCommands).toContain("pricing.price-lists.publish");
    expect(manifest.publicEvents).toContain("pricing.price-list-published.v1");
  });
});
...
describe("domain catalog", () => {
  it("keeps ERPNext parity references and operational surfaces visible", () => {
    ...
    expect(typeof publishPriceList).toBe("function");
    expect(typeof publishTaxRules).toBe("function");
    expect(typeof evaluateCommercialPolicy).toBe("function");
